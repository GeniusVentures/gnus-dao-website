...
export function ConnectWalletButton({
  ...
  const handleSelectWallet = async (connector: WalletConnector) => {
    ...
    try {
      ...
      if (
        ...
        message.includes("NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID")
      ) {
        setConfigError(message);
        handleCloseModal();
      } else {
        // Show error toast for user feedback
        toast.error("Connection Failed", message, 5000);
  ...
  if (configError) {
    return (
      <WalletConfigError error={configError} onRetry={handleRetryConnection} />
    );
  }
