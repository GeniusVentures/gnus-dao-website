...
const Web3ReduxProvider = lazy(() =>
  import("./Web3ReduxProvider").then((module) => ({
    default: module.Web3ReduxProvider,
  })),
);
...
export function LazyWeb3Provider({ children }: LazyWeb3ProviderProps) {
  return (
    <ErrorBoundary
      fallback={
        <Web3ErrorFallback
          error={new Error("Web3 Provider Error")}
    ...
    >
      <Suspense fallback={<Web3Loading />}>
        <Web3ReduxProvider>{children}</Web3ReduxProvider>
      </Suspense>
    </ErrorBoundary>
  );
}
